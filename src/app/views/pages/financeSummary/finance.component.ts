...
import { FinanceService, CollectionHistoryResponse, OverallFinanceSummary } from '../../../services/auth.service'; // Adjust path as needed
import { ChapterService } from '../../../services/auth.service'; // Adjust path as needed
import { common } from '../../../core/constants/common'; // Adjust path as needed
import { AppStorage } from '../../../core/utilities/app-storage'; // Adjust path as needed
import { swalHelper } from '../../../core/constants/swal-helper'; // Adjust path as needed
...
export class FinanceComponentOverall implements OnInit, AfterViewInit {
  ...
  overallSummary: OverallFinanceSummary | null = null;
  ...
  async fetchOverallSummary(): Promise<void> {
    this.loadingSummary = true;
    try {
      const response = await this.financeService.getOverallFinanceSummary();
  ...
  async fetchCollections(): Promise<void> {
    ...
    try {
      ...
      this.cdr.detectChanges();
      swalHelper.showToast('Failed to fetch collections', 'error');
    } finally {
      this.loading = false;
    }
  }
  ...
  async addCollection(form: any): Promise<void> {
    ...
    try {
      ...
      if (response.success) {
        ...
        this.resetForm();
        this.fetchCollections();
        this.fetchOverallSummary(); // Refresh summary after adding collection
      } else {
    ...
    try {
      ...
      if (result.isConfirmed) {
        ...
        if (response.success) {
          swalHelper.showToast(response.message, 'success');
          this.fetchCollections();
          this.fetchOverallSummary(); // Refresh summary after removing collection
        } else {
